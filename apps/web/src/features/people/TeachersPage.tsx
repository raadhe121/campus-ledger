...
import { createTeacherSchema, type CreateTeacherInput } from "@campus-ledger/validation-schemas";
import { useListTeachersQuery, useCreateTeacherMutation, useSetTeacherStatusMutation } from "./peopleApi";
...
export function TeachersPage() {
  ...
  const [setStatus, { isLoading: statusChanging }] = useSetTeacherStatusMutation();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold tracking-tight text-ink mt-1">Teachers</h1>
        <p className="text-sm text-muted mt-1">Assign them as a section's class teacher or to subjects later.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] items-start">
        <div className="rounded-2xl border border-line bg-surface overflow-hidden card-shadow">
          {isLoading ? (
            ...
            <p className="p-8 text-center text-sm text-rose-700">Could not load teachers.</p>
                  ...
                  <tbody className="divide-y divide-line/60">
                    {data.data.map(({ user, profile }) => (
                      <tr key={user.id} className="hover:bg-surface-2/40 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-muted hidden sm:table-cell text-xs">{profile.designation ?? "—"}</td>
                        <td className="px-4 py-3.5">
            ...
            <div className="p-10 text-center">
              ...
              <p className="font-medium text-ink mt-3">No teachers yet</p>
...
function CreateTeacherForm() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    setFormError(null);
    try {
      const { data } = await createTeacher(values).unwrap();
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="rounded-2xl border border-line bg-surface p-5 card-shadow grid gap-4 sticky top-20">
      <div className="flex items-center gap-2">
        <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center text-sm">＋</div>
        <p className="text-xs font-mono uppercase tracking-widest font-bold text-muted">Add a teacher</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="grid gap-1.5">
          <label htmlFor="t-firstName" className="text-xs font-semibold text-ink">
            First name
          </label>
          <input id="t-firstName" {...register("firstName")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
        ...
        <div className="grid gap-1.5">
          <label htmlFor="t-lastName" className="text-xs font-semibold text-ink">
            Last name
          </label>
          <input id="t-lastName" {...register("lastName")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
      ...
      <div className="grid gap-1.5">
        <label htmlFor="t-email" className="text-xs font-semibold text-ink">
          Email
        </label>
        <input id="t-email" type="email" {...register("email")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
      ...
      <div className="grid gap-1.5">
        <label htmlFor="t-phone" className="text-xs font-semibold text-ink">
          Phone <span className="text-muted font-normal">(optional)</span>
        </label>
        <input id="t-phone" {...register("phone")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm" />
      </div>
      <div className="grid gap-1.5">
        <label htmlFor="t-designation" className="text-xs font-semibold text-ink">
          Designation <span className="text-muted font-normal">(optional)</span>
        </label>
        <input id="t-designation" {...register("designation")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm" />
      </div>
      <div className="grid gap-1.5">
        <label htmlFor="t-department" className="text-xs font-semibold text-ink">
          Department <span className="text-muted font-normal">(optional)</span>
        </label>
        <input id="t-department" {...register("department")} className="rounded-xl border border-line bg-surface px-3.5 py-2.5 text-sm" />
      </div>
