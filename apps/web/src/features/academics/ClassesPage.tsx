...
import { createClassSchema, type CreateClassInput } from "@campus-ledger/validation-schemas";
import { useListAcademicYearsQuery, useListClassesQuery, useCreateClassMutation, useDeleteClassMutation } from "./academicsApi";
...
export function ClassesPage() {
  ...
  const { data, isLoading, error } = useListClassesQuery(yearId ? { academicYearId: yearId } : undefined, { skip: !yearId });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-ink mt-1">Classes</h1>
          <p className="text-sm text-muted mt-1">Grouped under an academic year — sections live inside a class.</p>
        </div>
        {years.length > 0 && (
          <select
            value={yearId}
      ...
      {years.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-line bg-surface p-8 text-center card-shadow">
          <div className="mx-auto h-12 w-12 rounded-2xl bg-surface-2 border border-line flex items-center justify-center text-xl">📚</div>
          <p className="font-medium text-ink mt-3">Create an academic year first</p>
          <p className="text-sm text-muted mt-1">Classes belong to a year.</p>
        </div>
      ) : (
        <div className="grid gap-6 lg:grid-cols-[1fr_340px] items-start">
          <div className="rounded-2xl border border-line bg-surface overflow-hidden card-shadow">
            {isLoading ? (
              ...
              <p className="p-8 text-center text-sm text-rose-700">Could not load classes.</p>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-line/60">
                      {data.data.map((cls) => (
                        <tr key={cls.id} className="hover:bg-surface-2/40">
                          <td className="px-4 py-3.5 font-semibold text-ink">{cls.name}</td>
                          <td className="px-4 py-3.5 text-muted font-mono text-xs">{cls.order}</td>
              ...
              <div className="p-10 text-center">
                <div className="mx-auto h-12 w-12 rounded-2xl bg-surface-2 border border-line flex items-center justify-center text-xl">📚</div>
                <p className="font-medium text-ink mt-3">No classes in this year yet</p>
                <p className="text-sm text-muted mt-1">Create one on the right.</p>
              </div>
            )}
          </div>

          <CreateClassForm academicYearId={yearId} />
        </div>
      )}
    </div>
  );
...
function CreateClassForm({ academicYearId }: { academicYearId: string }) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<Omit<CreateClassInput, "academicYearId">>({ resolver: zodResolver(createClassSchema.omit({ academicYearId: true })) });

  const onSubmit = handleSubmit(async (values) => {
    setFormError(null);
    try {
      await createClass({ ...values, academicYearId }).unwrap();
      reset();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="rounded-2xl border border-line bg-surface p-5 card-shadow grid gap-4 sticky top-20">
      <div className="flex items-center gap-2">
        <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 text-white flex items-center justify-center text-sm">＋</div>
        <p className="text-xs font-mono uppercase tracking-widest font-bold text-muted">New class</p>
      </div>
      <div className="grid gap-1.5">
        <label htmlFor="class-name" className="text-xs font-semibold text-ink">
          Name
        </label>
        <input
          id="class-name"
      ...
      <div className="grid gap-1.5">
        <label htmlFor="order" className="text-xs font-semibold text-ink">
          Order <span className="text-muted font-normal">(optional)</span>
        </label>
        <input
          id="order"
          type="number"
